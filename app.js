(function () {
  ...
  const signFlavors = {
    "牡羊座": "勢いよく始める力、直感的な行動力が出やすい領域です。",
    "牡牛座": "安心感、五感、じっくり育てる力がテーマになります。",
    "双子座": "好奇心旺盛で、フットワーク軽く情報を動かす領域です。",
    "蟹座": "安心感や身内とのつながり、心の居場所が鍵になります。",
    "獅子座": "自己表現、喜び、堂々と輝く力が出やすい領域です。",
    "乙女座": "整える力、分析、実務的な改善がテーマになります。",
    "天秤座": "調和、対話、バランス感覚を通して育つ領域です。",
    "蠍座": "深く向き合う力、絆、変容が起こりやすい領域です。",
    "射手座": "探求、学びの拡大、自由な視野が広がる領域です。",
    "山羊座": "責任、実績、長期的な目標づくりがテーマになります。",
    "水瓶座": "独自性、未来志向、仲間との新しいつながりが鍵になります。",
    "魚座": "感受性、癒し、見えないものへの共感が広がる領域です。"
  };
  let activeClientId = "";
  let selectedMonth = "2026-06";
  let selectedWholeSign = "牡羊座";
  let selectedWholeSignMonth = "2026-07";
  ...
  function wholeSignHouseForSign(sign, baseSign) {
    const signIndex = signs.indexOf(sign);
    const baseIndex = signs.indexOf(baseSign);
    if (signIndex < 0 || baseIndex < 0) return null;
    return ((signIndex - baseIndex + 12) % 12) + 1;
  }

  function houseLabel(startHouse, endHouse) {
    if (!startHouse || !endHouse) return "不明";
    return startHouse === endHouse ? `${startHouse}ハウス` : `${startHouse}→${endHouse}ハウス`;
  }

  function themeLabel(startHouse, endHouse) {
    if (!startHouse || !endHouse) return "テーマ不明";
    if (startHouse === endHouse) return houseThemes[startHouse];
    return `${houseThemes[startHouse]}から、${houseThemes[endHouse]}へ`;
  }

  function movesLabel(moves) {
    return (moves || []).map((move) => `${move.date} ${move.text}`).join("、") || "—";
  }

  function signForWholeSignHouse(house, baseSign) {
    const baseIndex = signs.indexOf(baseSign);
    if (baseIndex < 0) return "不明";
    return signs[(baseIndex + house - 1) % 12];
  }

  function wholeSignHouseMeaning(house, sign) {
    return `${houseThemes[house]}。${signFlavors[sign] || ""}`;
  }

  function wholeSignThemeLabel(startHouse, endHouse, startSign, endSign) {
    if (!startHouse || !endHouse) return "テーマ不明";
    if (startHouse === endHouse) return wholeSignHouseMeaning(startHouse, startSign);
    return `${startHouse}ハウス（${startSign}）：${wholeSignHouseMeaning(startHouse, startSign)} → ${endHouse}ハウス（${endSign}）：${wholeSignHouseMeaning(endHouse, endSign)}`;
  }

  function wholeSignDisplayDegree(pos) {
    if (!pos) return null;
  ...
  function buildWholeSignControls() {
    if (!els.wholeSignSelect || !els.wholeSignMonthSelect) return;
    els.wholeSignSelect.innerHTML = "";
    signs.forEach((sign) => els.wholeSignSelect.append(new Option(sign, sign)));
    els.wholeSignSelect.value = selectedWholeSign;

    els.wholeSignMonthSelect.innerHTML = "";
    wholeSignMonths.forEach((monthKey) => {
      const monthData = window.transitData && window.transitData[monthKey];
  ...
  function setFormData(data) {
    ...
    syncWholeSignFromNatalSun(data);
    render();
    renderWholeSignTable();
  }

  function syncWholeSignFromNatalSun(data) {
    const sunSign = data && data.natalPlanets && data.natalPlanets.sun && data.natalPlanets.sun.sign;
    if (!signs.includes(sunSign)) return;
    selectedWholeSign = sunSign;
    if (els.wholeSignSelect) els.wholeSignSelect.value = selectedWholeSign;
  }
  ...
  function drawWholeSignChart(monthData) {
    const svg = els.wholeSignChartSvg;
    if (!svg) return;
    svg.innerHTML = "";
    svg.append(svgEl("circle", { cx: 360, cy: 360, r: 310, fill: "#fffdf8", stroke: "#bda284", "stroke-width": 2 }));
    svg.append(svgEl("circle", { cx: 360, cy: 360, r: 232, fill: "none", stroke: "#d8c5ad", "stroke-width": 1.4 }));
    svg.append(svgEl("circle", { cx: 360, cy: 360, r: 122, fill: "#fbf4e9", stroke: "#d8c5ad" }));

    Array.from({ length: 12 }, (_, index) => index + 1).forEach((house) => {
      const cuspDegree = houseDisplayDegree(house);
      const sign = signForWholeSignHouse(house, selectedWholeSign);
      const a = point(cuspDegree, 122);
      const b = point(cuspDegree, 312);
      const houseLabelPoint = point(cuspDegree - 15, 104);
      const signLabelPoint = point(cuspDegree - 15, 276);
      svg.append(svgEl("line", { x1: a.x, y1: a.y, x2: b.x, y2: b.y, stroke: "#8f5d32", "stroke-width": house === 1 ? 3 : 1.4, opacity: 0.85 }));

      const houseText = svgEl("text", { x: houseLabelPoint.x, y: houseLabelPoint.y, "text-anchor": "middle", "dominant-baseline": "middle", fill: "#8f5d32", "font-size": 13, "font-weight": 800 });
      houseText.textContent = `${house}H`;
      svg.append(houseText);

      const signText = svgEl("text", { x: signLabelPoint.x, y: signLabelPoint.y, "text-anchor": "middle", "dominant-baseline": "middle", class: "svg-small" });
      signText.textContent = sign.replace("座", "");
      svg.append(signText);
    });

    const ascPoint = point(270, 334);
    const ascText = svgEl("text", { x: ascPoint.x, y: ascPoint.y, "text-anchor": "middle", "dominant-baseline": "middle", fill: "#8f5d32", "font-size": 14, "font-weight": 800 });
    ascText.textContent = `${selectedWholeSign} 1H`;
    svg.append(ascText);

    planetList(monthData).forEach((planet, index) => {
      const startDegree = wholeSignDisplayDegree(planet.start);
      const endDegree = wholeSignDisplayDegree(planet.end);
      if (!Number.isFinite(startDegree) || !Number.isFinite(endDegree)) return;
      const radius = 248 + (index % 3) * 13;
      drawMovement(svg, startDegree, endDegree, radius, String(planet.motion).includes("retrograde"));
      drawBody(svg, startDegree, radius, glyphs[planet.key] || planet.name[0], "#2f6f73", "#eaf6f5");
      drawBody(svg, endDegree, 276 + (index % 3) * 9, glyphs[planet.key] || planet.name[0], "#9a3f4b", "#faedf0");
    });

    (monthData.lunations || []).forEach((item) => {
      const degree = wholeSignDisplayDegree(item.position);
      if (Number.isFinite(degree)) drawBody(svg, degree, 292, item.type === "newMoon" ? "●" : "○", "#c9953d", "#fff3d6");
    });
  }

  function renderWholeSignTable() {
    if (!els.wholeSignTableBody || !els.wholeSignLunationList) return;
    const monthData = window.transitData && window.transitData[selectedWholeSignMonth];
    if (!monthData) return;
    ...
    planetList(monthData).forEach((planet) => {
      ...
      const row = document.createElement("tr");
      row.innerHTML = `
        <td data-label="天体">${esc(planet.name)}</td>
        <td data-label="星座の動き">${esc(positionLabel(planet.start))} → ${esc(positionLabel(planet.end))}</td>
        <td data-label="ハウス">${esc(houseLabel(startHouse, endHouse))}</td>
        <td data-label="星座移動日">${esc(movesLabel(planet.moves))}</td>
        <td data-label="逆行・順行">${esc(motionLabel(planet))}</td>
        <td data-label="ひとことテーマ">${esc(wholeSignThemeLabel(startHouse, endHouse, planet.start.sign, planet.end.sign))}</td>`;
      els.wholeSignTableBody.append(row);
    });

    els.wholeSignLunationList.innerHTML = "";
    (monthData.lunations || []).forEach((item) => {
      const house = wholeSignHouseForSign(item.position.sign, selectedWholeSign);
      const card = document.createElement("article");
      card.className = "whole-sign-lunation-card";
      card.innerHTML = `
        <strong>${esc(item.label)} ${esc(item.date)} ${esc(item.time)}</strong>
        <div>${esc(positionLabel(item.position))}／${esc(selectedWholeSign)}さんから見て${house ? `${house}ハウス` : "ハウス不明"}</div>
        <div class="small">テーマ：${esc(house ? wholeSignHouseMeaning(house, item.position.sign) : "テーマ不明")}</div>`;
      els.wholeSignLunationList.append(card);
    });
  }

  function renderWholeSignHouseMap() {
    if (!els.wholeSignHouseMap) return;
    els.wholeSignHouseMap.innerHTML = "";
    Array.from({ length: 12 }, (_, index) => index + 1).forEach((house) => {
      const item = document.createElement("div");
      item.className = "whole-sign-house-card";
      item.innerHTML = `
        <strong>${house}ハウス</strong>
        <span>${esc(signForWholeSignHouse(house, selectedWholeSign))}</span>
        <p>${esc(wholeSignHouseMeaning(house, signForWholeSignHouse(house, selectedWholeSign)))}</p>`;
      els.wholeSignHouseMap.append(item);
    });
  }
  ...
  function bindEvents() {
    document.addEventListener("input", (event) => {
      ...
      if (event.target.tagName === "SELECT") {
        ...
        if (event.target.id === "natal-0-sign" && signs.includes(event.target.value)) {
          selectedWholeSign = event.target.value;
          if (els.wholeSignSelect) els.wholeSignSelect.value = selectedWholeSign;
          renderWholeSignTable();
        }
      }
    ...
    if (els.wholeSignSelect) {
      els.wholeSignSelect.addEventListener("change", () => {
        selectedWholeSign = els.wholeSignSelect.value;
        renderWholeSignTable();
      });
    }
    if (els.wholeSignMonthSelect) {
      els.wholeSignMonthSelect.addEventListener("change", () => {
        selectedWholeSignMonth = els.wholeSignMonthSelect.value;
        renderWholeSignTable();
      });
    }
  }
